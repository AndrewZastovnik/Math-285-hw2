...
def KNN(I, L, x, k):
    ...
    for rowsx in range(0, sizex):
        dists = cdist(I, np.atleast_2d(x[rowsx]), metric='cityblock')
        # Now we should have all our distances in our dist array
        # The next step is to use this info to classify each unknown obj
        k_smallest = np.argpartition(dists,tuple(range(1,k)),axis=None)
        for i in range(0,k):
            print(L[k_smallest[:i+1]])
...
label = KNN(train_Images, train_Labels, test_Images, 10)
